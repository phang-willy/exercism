...
export function buildBirthdaySign(age) {
  const sentence = (age >= 50) ? "mature" : "young"

  return `Happy Birthday! What a ${sentence} fellow you are.`
}
...
export function costOf(sign, currency) {
  const BASED_PRICE = 20
  const PRICE_BY_LETTER = 2
  const price = Number(sign.length * PRICE_BY_LETTER + BASED_PRICE).toFixed(2)
